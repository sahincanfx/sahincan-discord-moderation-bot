...
if (!üye) return message.channel.send(embed.setDescription("Geçerli bir kullanıcı belirtmelisin! [`!vmute @SahincanID 5m Düzeni Bozma`]"));
...
if (!süre || !ms(süre)) return message.channel.send(embed.setDescription("Geçerli bir süre belirtmelisin! [`!vmute @SahincanID 5m Düzeni Bozma`]"));
...
message.guild.channels.cache.get(conf.muteLog).send(embed.setDescription(`${üye} isimli kullanıcı ${message.author} tarafından "${süre}" süresince seste mutelendi! [\`${sebep}\`]`));
