...
if (!conf.jailSorumlusu.some(qwe => message.member.roles.cache.has(qwe)) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed.setDescription(`Yeterli yetkilere sahip değilsin!`))

let üye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
let süre = args[1];
let sebep = args.splice(2).join(" ") || "Sebep Belirtilmedi!"
if (!üye) return message.channel.send(embed.setDescription("Geçerli bir kullanıcı belirtmelisin! [`!jail @SahincanID 5m Düzeni Bozma`]"));
...
if (!süre || !ms(süre)) return message.channel.send(embed.setDescription("Geçerli bir süre belirtmelisin! [`!jail @SahincanID 5m Düzeni Bozma`]"));


if(üye) üye.roles.set([conf.jailRolu]);
...
message.guild.channels.cache.get(conf.jailLog).send(embed.setDescription(`${üye} isimli kullanıcı ${message.author} tarafından "${süre}" süresince cezalıya atıldı! [\`${sebep}\`]`));
...
message.guild.channels.cache.get(conf.jailLog).send(embed.setDescription(`${üye} kullanıcısının cezası süresi bittiği için kaldırıldı [\`${sebep}\`]`));
}, ms(süre));
};
...
exports.config = {
  name: "jail",
  guildOnly: true,
  aliases: ["tempjail", "cezalı", "cezali"],
};