...
if (!üye) return message.channel.send(embed.setDescription("Geçerli bir kullanıcı belirtmelisin! [`!unceza @SahincanID`]"));
...
if (muteli && üye.roles.cache.has(conf.muteRolu)) muteli = "Şu anda muteli!";
if (!muteli && !üye.roles.cache.has(conf.muteRolu)) muteli = "Şu anda muteli değil!";

if (jailli && üye.roles.cache.has(conf.jailRolu)) jailli = "Şu anda cezalıda!";
if (!jailli && !üye.roles.cache.has(conf.jailRolu)) jailli = "Şu anda cezalıda değil!";
...
üye.roles.remove(conf.muteRolu);
})
}
